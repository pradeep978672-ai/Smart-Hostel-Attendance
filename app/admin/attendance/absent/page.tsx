...
export default function AbsentStudentsPage() {
  ...
  const fetchAbsent = useCallback(async (dateToLoad: string = selectedDate) => {
    setIsLoading(true);
    try {
      ...
      console.error('Error fetching absent students:', e);
    } finally {
      setIsLoading(false);
    }
  }, [selectedDate]);
  ...
  useEffect(() => {
    fetchAbsent(selectedDate);
  }, [selectedDate, fetchAbsent]);
  ...
  // Filter absent / unsubmitted records based on search query
  const filteredRecords = useMemo(() => {
    const absentOnly = records.filter((r) => r.status === 'ABSENT' || r.status === 'NOT_SUBMITTED');
    if (!searchQuery.trim()) return absentOnly;
    const q = searchQuery.trim().toLowerCase();
    return absentOnly.filter((rec) => {
  ...
  // Group filtered absent records by Department
  ...
  const totalAbsentCount = records.filter((r) => r.status === 'ABSENT' || r.status === 'NOT_SUBMITTED').length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight flex items-center gap-2">
            <XCircle className="w-6 h-6 text-rose-400" /> Absent Students Directory
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Department-wise collapsible directory of hostel residents who have not submitted attendance for {formatDateDisplay(selectedDate)}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={() => fetchAbsent(selectedDate)}
            disabled={isLoading}
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-slate-900/90 p-4 border border-slate-800 rounded-2xl">
        <div className="flex-1">
          <SearchBar
            ...
            placeholder="Search absent students by Name, Roll Number, Department, or Room..."
          />
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <div className="flex items-center gap-2 px-3.5 py-2 bg-slate-950 rounded-xl border border-slate-800">
            ...
            <span className="text-xs text-slate-300">
              Total Absent: <strong className="text-rose-400 font-bold">{filteredRecords.length}</strong>
              {searchQuery.trim() && (
                <span className="text-slate-500 font-normal"> (of {totalAbsentCount})</span>
              )}
            </span>
          </div>
      ...
      {departmentGroups.length === 0 ? (
        <div className="w-full p-8 text-center bg-slate-900/80 rounded-2xl border border-slate-800 text-slate-400 text-xs italic">
          No absent students found matching current criteria.
        </div>
      ) : (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[11px] text-slate-400">
                        {isExpanded ? 'Click to collapse student list' : 'Click to view absent students'}
                      </span>
                    </div>
                  </div>

                  <div className="flex items-center gap-3">
                    <span className="text-xs font-semibold px-3 py-1 bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-xl flex items-center gap-1.5">
                      <XCircle className="w-3.5 h-3.5" />
                      {group.students.length} {group.students.length === 1 ? 'Student Absent' : 'Students Absent'}
                    </span>
                    <div
